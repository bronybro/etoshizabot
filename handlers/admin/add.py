...
@dp.message_handler(state=Stock.name)
...
@dp.message_handler(state=Stock.description)
...
@dp.message_handler(lambda message: not message.text.isdigit(), state=Stock.price)
async def product_price_invalid(message: Message):
    return await message.reply('Error! The price must be a number. For example 100 or 100.00')


@dp.message_handler(lambda message: message.text.isdigit(), state=Stock.price)
...
# TODO product photo
@dp.message_handler(state=Stock.photo, content_types=types.ContentType.PHOTO)
...
@dp.callback_query_handler(submit_callback.filter(decision=['submit']), state=Stock.submit)
async def submit_product(call: CallbackQuery, state: FSMContext):
    ...
    tables.cur.execute(
        f'INSERT INTO products VALUES("{data["name"]}","{data["description"]}","{data["price"]}","{data["photo"]}")')
    tables.conn.commit()
    await call.message.answer(text='The product list has been updated')
    await state.reset_state()


@dp.callback_query_handler(submit_callback.filter(decision=['cancel']),state=Stock.submit)
